#!/usr/bin/env bash
# 运行 ShardingSphere 分库分表证据测试（2 库 × 2 表），原始输出固化到 output/ 作为面试证据。
# 前置：本地 MySQL 9 已启动，root 密码通过 MYSQL_PASSWORD 传入（默认 783421 仅本机开发库）。
set -uo pipefail

cd /c/Project/QuestionBank
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="output/sharding_evidence_${STAMP}.log"

export MYSQL_SHARDING_EVIDENCE=true
...
./scripts/mvn.sh -B test -Dtest=ShardingEvidenceTest -Dsurefire.failIfNoSpecifiedTests=false 2>&1 | tee "$OUT"
STATUS=${PIPESTATUS[0]}
