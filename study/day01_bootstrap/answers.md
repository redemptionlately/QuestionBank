...
public class BankService{
    ...
    public BankService(QuestionBankRepository bankRepository) {
        this.bankRepository = bankRepository;
    }

    @Transactional
    public QuestionBank createBank(String name) {
        if (name == null || name.isBlank()) throw new IllegalArgumentException("题目名称不能为空");
        return bankRepository.save(new QuestionBank(name.trim()));
    }
}
...
public class BankController {
    ...
    public BankResponse createBank(@Valid @RequestBody CreateBankRequest request) {
        QuestionBank bank = service.createBank(CurrentUser.require(), request.name())
        return new BankResponse(bank.getName());
    }

    public record CreateBankRequest(@NotBlank @Size(max = 160) String name) {}
    public record BankResponse (Long id, String name) {}
}
