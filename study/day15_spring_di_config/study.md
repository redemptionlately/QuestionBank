...
- 构造器注入源码形态是：
  ...
  ```
- 源码索引（MustRemember）：[SecurityConfig.java](../../src/main/java/com/allen/questionbank/auth/SecurityConfig.java) 第 23-29 行的 `@Bean`、构造器参数和 SecurityFilterChain 创建
...
- 源码索引（MustUnderstand）：[application.yml](../../src/main/resources/application.yml) 的环境变量占位符和 profile 覆盖；[QuestionBankApplication.java](../../src/main/java/com/allen/questionbank/QuestionBankApplication.java) 第 7-12 行的组件扫描入口
- 外部源码索引（MustUnderstand）：[Spring AOP Proxying](https://docs.spring.io/spring-framework/reference/core/aop/proxying.html) 的代理类型与 self-invocation；[Spring Transaction Management](https://docs.spring.io/spring-framework/reference/data-access/transaction.html) 的代理边界
