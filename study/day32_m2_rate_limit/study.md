...
- [RateLimitFilter.java](../../src/main/java/com/allen/questionbank/common/RateLimitFilter.java) 第 17-57 行实现按远端地址和 URI 的固定窗口计数，超过容量返回 429 与 `Retry-After`；计数器使用 `ConcurrentHashMap` 和 `AtomicInteger`
...
- 令牌桶的计算骨架是：
  ...
  if allow: tokens -= 1
  ```
- 外部源码索引（MustRemember）：[Bucket4j](https://bucket4j.com/8.15.0/toc.html) 的 `BucketConfiguration`/`Bandwidth`；[HTTP 429](https://httpwg.org/specs/rfc6585.html#status-429) 的响应契约
...
- 外部源码索引（MustUnderstand）：[Redis Lua scripting](https://redis.io/docs/latest/develop/programmability/eval-intro/) 的共享计数原子性和失败边界
