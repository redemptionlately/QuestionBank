...
- [RequestMetrics.java](../../src/main/java/com/allen/questionbank/common/RequestMetrics.java) 第 6-23 行用三个 `AtomicLong` 记录请求总数、失败数和累计纳秒；`record` 封装同步调用，`request/failure/latency` 供过滤器组合使用
- [MetricsController.java](../../src/main/java/com/allen/questionbank/common/MetricsController.java) 第 8-17 行通过 `GET /api/metrics` 导出 JSON；[RateLimitFilter.java](../../src/main/java/com/allen/questionbank/common/RateLimitFilter.java) 第 33-55 行说明请求计数、429/异常失败计数和 finally 计时的接入点；[InfrastructureConfig.java](../../src/main/java/com/allen/questionbank/common/InfrastructureConfig.java) 第 27-28 行注册单例指标 Bean
