...
- cache-aside 的代码骨架是：
  ...
  return loaded;
  ```
- 外部源码索引（MustRemember）：[RedisTemplate API](https://docs.spring.io/spring-data/redis/reference/redis/template.html) 的 `opsForValue().get/set`；[Redis SET 命令](https://redis.io/docs/latest/commands/set/) 的 `NX/EX` 参数
...
- 外部源码索引（MustUnderstand）：[Redis 事务与脚本](https://redis.io/docs/latest/develop/using-commands/transactions/) 的 MULTI/EXEC/Lua 原子边界
