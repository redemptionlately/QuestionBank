...
- 外部源码索引（会背会写）：[Spring StateMachine](https://docs.spring.io/spring-statemachine/docs/current/reference/) 的 state/event/transition 配置模型

# 必须理解

- 审核是状态机、权限和追加审计的组合；解析只产生候选，人工决定是否进入正式题库
- 并发审核需要 `entity_version` 检查或悲观行锁，旧页面不能覆盖新决定；冲突返回 409
- 外部源码索引（必须理解）：[Spring StateMachine guards/actions](https://docs.spring.io/spring-statemachine/docs/current/reference/#sm-guards) 的守卫与副作用边界
