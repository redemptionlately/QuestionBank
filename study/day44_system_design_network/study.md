...
- 外部源码索引（会背会写）：[Google SRE SLO](https://sre.google/workbook/implementing-slos/) 的 SLI/SLO/error budget；[HTTP semantics](https://httpwg.org/specs/rfc9110.html) 的 timeout/retry/status 语义

# 必须理解

- 系统设计的完整模型是问题、约束、数据流、状态机、失败模式、容量假设、观测指标、验证方式和替代方案
- 拆服务需要流量、团队、故障或数据所有权证据；没有证据时本地事务更简单、更容易证明正确
- 外部源码索引（必须理解）：[Google SRE capacity planning](https://sre.google/workbook/capacity-planning/) 的容量、排队和错误预算关系
- 外部源码索引（会背会写）：[RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293)、[RFC 9114 HTTP/3](https://www.rfc-editor.org/rfc/rfc9114) 的连接与超时语义；[curl manual](https://curl.se/docs/manpage.html) 的 verbose/timeout 选项
- 外部源码索引（必须理解）：[RFC 8305 Happy Eyeballs](https://www.rfc-editor.org/rfc/rfc8305) 的 DNS/连接选择；[nginx proxy headers](https://nginx.org/en/docs/http/ngx_http_proxy_module.html)
