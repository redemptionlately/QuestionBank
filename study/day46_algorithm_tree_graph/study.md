...
- BFS 代码骨架是：
  ...
  ```
- 外部源码索引（MustRemember）：[Java ArrayDeque](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/ArrayDeque.html)、[PriorityQueue](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/PriorityQueue.html)；[树/图 INDEX](../../../AI_Infra/Phases_book/algorithm/4_链表_树与_LCA/INDEX.md)

# MustUnderstand

- 树是无环连通图；递归深度过大可能耗尽 Java 栈，图搜索必须明确有向/无向、重复边、断开图和权值条件
- 外部源码索引（MustUnderstand）：[CP-Algorithms BFS](https://cp-algorithms.com/graph/breadth-first-search.html)、[Dijkstra](https://cp-algorithms.com/graph/dijkstra.html)、[DSU](https://cp-algorithms.com/data_structures/disjoint_set_union.html)
