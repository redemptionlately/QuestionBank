...
- 滑动窗口代码骨架是：
  ...
  ```
- 外部源码索引（MustRemember）：[Java HashMap API](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/HashMap.html) 的 `get/put/merge`；[数组/窗口 INDEX](../../../AI_Infra/Phases_book/algorithm/1_数组_窗口_前缀和与选择/INDEX.md) 的题面与变式

# MustUnderstand

- 滑窗正确性依赖窗口不变量；重复值、空窗口、负数和边界决定窗口能否单调收缩，不能机械套模板
- 外部源码索引（MustUnderstand）：[CP-Algorithms prefix sums](https://cp-algorithms.com/data_structures/prefix_sum.html) 与 [two pointers](https://cp-algorithms.com/others/two_pointers.html) 的不变量证明
