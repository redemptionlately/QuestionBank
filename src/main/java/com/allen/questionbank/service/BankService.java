...
import com.allen.questionbank.controller.BankController;
...
public class BankService {
    ...
    public BankService(QuestionBankRepository bankRepository, PaperVersionRepository papers,
                       QuestionVersionRepository questions, ObjectMapper objectMapper,
                       ExpiringCache<String, List<PaperVersion>> publishedCache) {
        this.bankRepository = bankRepository; this.papers = papers; this.questions = questions; this.objectMapper = objectMapper;
    ...
    public QuestionBank createBank(ApiTokenFilter.AuthPrincipal user, String name, String description) {
        ...
        return bankRepository.save(new QuestionBank(user.userId(), name.trim(), description));
    }
    ...
    public PaperVersion createDraft(ApiTokenFilter.AuthPrincipal user, Long bankId, String title,
        ...
        QuestionBank bank = bankRepository.findById(bankId).orElseThrow(() -> notFound("题库不存在"));
    ...
    public PaperVersion publish(ApiTokenFilter.AuthPrincipal user, Long paperId) {
        ...
        QuestionBank bank = bankRepository.findById(paper.getBankId()).orElseThrow(() -> notFound("题库不存在"));
        if (!bank.getOwnerId().equals(user.userId())) throw forbidden();
